...
import com.fun.dante.wordbook.MyApplication;
import com.google.gson.Gson;
import Gson.ChineseToEnglish;
import com.fun.dante.wordbook.MainActivity;
...
public class resolve {
     ...
     * */
    public static void EnglishToChineseXMLWithPull(String result) {
    ...
    public static void parseJinshanChineseToEnglishJSONWithGson(String result) {
        ...
        try {
            Gson gson = new Gson();
            ChineseToEnglish translate = gson.fromJson(result, ChineseToEnglish.class);
            ...
            for (ChineseToEnglish.SymbolsBean voiceMsg : translate.getSymbols()){
                voiceText += voiceMsg.getWord_symbol()+" , ";
                voiceUrlText += voiceMsg.getSymbol_mp3();
                for(ChineseToEnglish.SymbolsBean.PartsBean meanMsg : voiceMsg.getParts()){
                    meanText += meanMsg.getPart_name()+": ";
                    for(ChineseToEnglish.SymbolsBean.PartsBean.MeansBean mean :meanMsg.getMeans()){
